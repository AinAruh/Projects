...
public sealed class AssetsController(IAssetService service) : ControllerBase
{
    ...
    [HttpPost]
    public async Task<ActionResult<AssetDto>> Create([FromBody] SaveAssetRequest request, CancellationToken ct)
    {
    ...
    [HttpPut("{id:guid}")]
    public async Task<ActionResult<AssetDto>> Update(Guid id, [FromBody] SaveAssetRequest request, CancellationToken ct)
    {
        var asset = await service.UpdateAsync(id, request, ct);
        return asset is null ? NotFound() : Ok(asset);
    }
